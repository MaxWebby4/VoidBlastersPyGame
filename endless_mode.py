...
# Add Game State Class for Power-Up Effects (such as double points)
class GameState:
    def __init__(self):
...
# Endless Mode function (spawns both asteroid and shooting enemies)
def endless_mode(screen):
    score = 0
    running = True
    ...
    last_enemy_spawn = pygame.time.get_ticks()  # Initialize it here
    ...
    while running:
        ...
        # Control power-up spawning (separately from enemy spawning)
        if now - last_powerup_spawn > powerup_respawn_timer:
            ...
            last_powerup_spawn = now  # Reset the power-up spawn timer
        ...
        for power_up in power_up_hits:
            if isinstance(power_up, SpeedPowerUp):
                player.speed = player.base_speed * 2  # Set speed to double the base speed
                player.speed_powerup_time = pygame.time.get_ticks()  # Reset the power-up timer
            else:
