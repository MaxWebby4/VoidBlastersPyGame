...
class ShootingEnemy(pygame.sprite.Sprite):
    def __init__(self, player):
        ...
        self.speed = 5
        self.shoot_delay = random.randint(1500, 4000)  # Random delay between shots (1500ms to 4000ms)
    ...
    def update(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            self.shoot()
            self.last_shot = now
            self.shoot_delay = random.randint(1500, 4000)  # Re-randomize the delay
...
def level_2(screen):
    ...
    for sprite in all_sprites:
        if isinstance(sprite, ShootingEnemy) or isinstance(sprite, EnemyBullet):
    ...
    while running:
        ...
        # Spawn enemies (limit to 6 at a time)
        if len(enemies) < 6 and random.random() < 0.02:  # Adjust for difficulty
            enemy = ShootingEnemy(player)  # Pass player as argument
